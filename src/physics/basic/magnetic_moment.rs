    ...
    fn add(self, rhs: f64) -> Self::Output {
        match self.default_type {
            ...
            MagneticMomentType::MillAM2 => {
                let v = self.as_mill_am2() + rhs;
                Self::from_mill_am2(v)
            }
            MagneticMomentType::MicroAM2 => {
                let v = self.as_micro_am2() + rhs;
                Self::from_micro_am2(v)
            }
            MagneticMomentType::NanoAM2 => {
                let v = self.as_nano_am2() + rhs;
                Self::from_nano_am2(v)
            }
            MagneticMomentType::JPerTesla => {
                let v = self.as_j_per_tesla() + rhs;
                Self::from_j_per_tesla(v)
            }
            MagneticMomentType::MillJPerTesla => {
                let v = self.as_mill_j_per_tesla() + rhs;
                Self::from_mill_j_per_tesla(v)
            }
            MagneticMomentType::MicroJPerTesla => {
                let v = self.as_micro_j_per_tesla() + rhs;
                Self::from_micro_j_per_tesla(v)
            }
            MagneticMomentType::NanoJPerTesla => {
                let v = self.as_nano_j_per_tesla() + rhs;
                Self::from_nano_j_per_tesla(v)
            }
        }
    }
}
